		...
		<form>
		    ...
		    <select name="brand">
		    	<option selected="selected" value="" hidden="">None</option>
		      	...
		      	<option value="pineng">Pineng</option>
		    </select>
		    <br>
...
if ($var1 == '' && $var2 == '') {
	$sql = "SELECT * FROM product WHERE categoriesId=0";
	$query = $dbh -> prepare($sql);
} else if ($var1 != '' && $var2 != '') {
	$sql = "SELECT * FROM product WHERE categoriesId=0 AND brand=:var1 AND unitPrice >= :price1 AND unitPrice <= :price2";
	$query = $dbh -> prepare($sql);
	...
	$sql = "SELECT * FROM product WHERE categoriesId=0 AND brand=:var1";
	$query = $dbh -> prepare($sql);
	$query -> bindParam(":var1", $var1, PDO::PARAM_STR);
} else {
	$sql = "SELECT * FROM product WHERE categoriesId=0 AND unitPrice >= :price1 AND unitPrice <= :price2";
	$query = $dbh -> prepare($sql);
	$query -> bindParam(":price1", $price1, PDO::PARAM_INT);
	$query -> bindParam(":price2", $price2, PDO::PARAM_INT);
}
