...
if ($var1 == '' && $var2 == '') {
	$sql = "SELECT * FROM product WHERE categoriesId=1";
	$query = $dbh -> prepare($sql);
} else if ($var1 != '' && $var2 != '') {
	$sql = "SELECT * FROM product WHERE categoriesId=1 AND brand=:var1 AND unitPrice >= :price1 AND unitPrice <= :price2";
	$query = $dbh -> prepare($sql);
	...
	$sql = "SELECT * FROM product WHERE categoriesId=1 AND brand=:var1";
	$query = $dbh -> prepare($sql);
	$query -> bindParam(":var1", $var1, PDO::PARAM_STR);
} else {
	$sql = "SELECT * FROM product WHERE categoriesId=1 AND unitPrice >= :price1 AND unitPrice <= :price2";
	$query = $dbh -> prepare($sql);
	$query -> bindParam(":price1", $price1, PDO::PARAM_INT);
	$query -> bindParam(":price2", $price2, PDO::PARAM_INT);
}
